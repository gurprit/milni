...
export async function POST(request:NextRequest,{params}:{params:Promise<{slug:string}>}){
 try{const{slug}=await params;const session=await currentGuestSession();const organiserSession=await currentOrganiserSession();const organiser=await organiserCanAccessWedding(organiserSession,slug);const wid=await weddingId(slug);if(!wid||(!organiser&&(!session||session.weddingId!==wid||session.weddingSlug!==slug)))return NextResponse.json({ok:false,error:'Join this wedding before adding photos.'},{status:401});const body=await request.json() as {id:string;albumId?:string;eventId?:string;objectKey:string;caption?:string;uploadedBy?:string};const id=wid;let albumId:string|null=null;let eventId:string|null=null;if(body.albumId){const album=(await d1Query<{id:string;event_id:string|null}>('SELECT id,event_id FROM photo_albums WHERE id=? AND wedding_id=? LIMIT 1',[body.albumId,id]))[0];if(album){albumId=album.id;eventId=album.event_id??null}}if(!eventId&&body.eventId){const event=(await d1Query<{id:string}>('SELECT id FROM events WHERE id=? AND wedding_id=? LIMIT 1',[body.eventId,id]))[0];if(event)eventId=event.id}await d1Execute('INSERT INTO photos (id,wedding_id,album_id,event_id,object_key,caption,uploaded_by_name) VALUES (?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET album_id=excluded.album_id,event_id=excluded.event_id,caption=excluded.caption,uploaded_by_name=excluded.uploaded_by_name',[body.id,id,albumId,eventId,body.objectKey,body.caption??'',body.uploadedBy??'Wedding guest']);return NextResponse.json({ok:true})}catch(error){return NextResponse.json({ok:false,error:error instanceof Error?error.message:'Could not save photo'},{status:500})}
}
