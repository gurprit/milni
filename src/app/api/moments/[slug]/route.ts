...
export async function POST(request:Request,{params}:{params:Promise<{slug:string}>}){
 ...
 if(body.action==='like'){if(!body.postId)return NextResponse.json({error:'Post required'},{status:400});const exists=await d1Query<{actor_id:string}>('SELECT actor_id FROM moments_likes WHERE post_id=? AND actor_id=?',[body.postId,me.id]);if(exists.length)await d1Execute('DELETE FROM moments_likes WHERE post_id=? AND actor_id=?',[body.postId,me.id]);else await d1Execute('INSERT INTO moments_likes(post_id,actor_id,created_at) VALUES(?,?,?)',[body.postId,me.id,new Date().toISOString()]);return NextResponse.json({ok:true,liked:!exists.length})}
