...
export async function POST(request:Request,{params}:{params:Promise<{slug:string}>}){
 ...
 if(body.action==='comment'){const message=String(body.message||'').trim().slice(0,1000);const mediaUrl=String(body.mediaUrl||'').trim()||null;if(!body.postId||(!message&&!mediaUrl))return NextResponse.json({error:'Add a comment, photo or GIF.'},{status:400});const comment={id:crypto.randomUUID(),post_id:body.postId,author_id:me.id,author_name:me.name,author_role:me.role,message,media_url:mediaUrl,created_at:new Date().toISOString()};await d1Execute('INSERT INTO moments_comments(id,post_id,author_id,author_name,author_role,message,media_url,created_at) VALUES(?,?,?,?,?,?,?,?)',[comment.id,comment.post_id,comment.author_id,comment.author_name,comment.author_role,comment.message,comment.media_url,comment.created_at]);const target=(await d1Query<{author_id:string}>('SELECT author_id FROM moments_posts WHERE id=? AND wedding_id=?',[body.postId,id]))[0];const recipient=target?.author_id.startsWith('guest:')?target.author_id.slice(6):'';if(recipient&&target.author_id!==me.id)await publishWeddingNotification({weddingId:id,type:'moment_comment',title:`${me.name} commented on your Moment`,message:message||'Added a photo or GIF to your post.',url:`/wedding/${slug}/moments#moment-${body.postId}`,sourceId:comment.id,recipientGuestId:recipient});return NextResponse.json({ok:true,comment})}
 const message=String(body.message||'').trim().slice(0,2000);const mediaUrl=String(body.mediaUrl||'').trim()||null;if(!message&&!mediaUrl)return NextResponse.json({error:'Share a message, photo or GIF.'},{status:400});const post={id:crypto.randomUUID(),author_id:me.id,author_name:me.name,author_role:me.role,message,media_url:mediaUrl,event_id:String(body.eventId||'').trim()||null,event_name:String(body.eventName||'').trim()||null,created_at:new Date().toISOString()};
