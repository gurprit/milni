...
export default function AddGuestsModal({slug,onClose,onAdd}:{slug:string;onClose:()=>void;onAdd:(guests:Guest[])=>Promise<boolean>}){
 ...
 {mode==='household'&&<div className={styles.addGuestForm}><label className={styles.full}>Household / family name<input value={household.group} onChange={e=>setHousehold({...household,group:e.target.value})} placeholder="e.g. Kapoor family"/></label><label className={styles.full}>Guest names<textarea autoFocus rows={6} value={household.names} onChange={e=>setHousehold({...household,names:e.target.value})} placeholder={"Aman Kapoor\nSimran Kapoor\nNina Kapoor"}/><small>One person per line. Each person remains their own guest account.</small></label><label>Family fallback email<input type="email" value={household.email} onChange={e=>setHousehold({...household,email:e.target.value})} placeholder="Optional"/></label><label>Family fallback phone<input value={household.phone} onChange={e=>setHousehold({...household,phone:e.target.value})} placeholder="Optional"/></label><div className={styles.full}><strong>How family access works</strong><p>If this fallback contact matches several guests, MILNI asks “Who are you?”. If you later give a family member their own phone number or email, that unique contact takes them straight into their own guest account. These details never grant organiser access.</p></div><label>Side<select value={household.side} onChange={e=>setHousehold({...household,side:e.target.value as GuestSide})}>{SIDES.map(side=><option key={side}>{side}</option>)}</select></label><label>Dietary note<input value={household.dietary} onChange={e=>setHousehold({...household,dietary:e.target.value})} placeholder="Optional shared note"/></label><label className={styles.full}>Private organiser notes<textarea rows={2} value={household.notes} onChange={e=>setHousehold({...household,notes:e.target.value})}/></label></div>}

 {mode==='import'&&<div className={styles.importGuests}><div className={styles.importHelp}><strong>Paste from Excel or Google Sheets</strong><p>Copy rows straight from a spreadsheet, or upload a CSV/TSV file. Recognised columns include Name, Group, Email, Phone, Side, Dietary, Plus One, Notes and Status. Individual Email or Phone values give direct guest access. Repeated family contact details are allowed and will show the “Who are you?” chooser.</p><label className={styles.fileButton}>Choose CSV / TSV<input type="file" accept=".csv,.tsv,.txt,text/csv,text/tab-separated-values" onChange={e=>{const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>setImportText(String(reader.result||''));reader.readAsText(file);e.currentTarget.value=''}}/></label></div><textarea className={styles.importTextarea} rows={9} value={importText} onChange={e=>setImportText(e.target.value)} placeholder={"Name\tGroup\tEmail\tPhone\nAman Kapoor\tKapoor family\taman@example.com\t07123 456789"}/>{preview.length>0&&<div className={styles.importPreview}><strong>{preview.length} guest{preview.length===1?'':'s'} ready to import</strong><span>{preview.slice(0,5).map(g=>g.name).join(' · ')}{preview.length>5?` · +${preview.length-5} more`:''}</span></div>}</div>}
