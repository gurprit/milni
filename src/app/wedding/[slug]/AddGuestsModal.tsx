...
export default function AddGuestsModal({slug,onClose,onAdd}:{slug:string;onClose:()=>void;onAdd:(guests:Guest[])=>Promise<boolean>}){
 ...
 const[household,setHousehold]=useState({group:'',names:'',side:'Both' as GuestSide,dietary:'',notes:''});
 const[importText,setImportText]=useState('');
 const preview=useMemo(()=>importedGuests(importText),[importText]);

 const build=()=>{
  if(mode==='single')return single.name.trim()?[makeGuest(single)]:[];
  if(mode==='household')return household.names.split(/\n+/).map(v=>v.trim()).filter(Boolean).map((name,index)=>makeGuest({name,group:household.group||'Family',side:household.side,dietary:household.dietary,notes:household.notes},index));
  return preview;
 };
 ...
 const reset=()=>{setAdded([]);setInviteLinks([]);setError('');setSingle({name:'',group:'Friends',email:'',phone:'',side:'Both',dietary:'',plusOne:'',notes:''});setHousehold({group:'',names:'',side:'Both',dietary:'',notes:''});setImportText('')};
