...
type CoachJourney=TravelItem&{trackerGuestIds?:string[];trackerNames?:string[];canTrack?:boolean;isCurrentTracker?:boolean;live?:CoachLive};
  ...
  try{
   ...
   const response=await fetch(`/api/coaches/${encodeURIComponent(slug)}`,{
    ...
    body:JSON.stringify({journeys:next.map(item=>({
     ...
     trackerGuestIds:item.trackerGuestIds??[]
    }))})
   });
 ...
 const loadCoachData=useCallback(async()=>{
  try{
   ...
   if(journeys.length){
    ...
    const initial=travelRef.current.map(item=>({...item,trackerGuestIds:[],trackerNames:[],canTrack:true,isCurrentTracker:false,live:idleLive}));
 ...
 const persistTravel=(nextItems:TravelItem[])=>{
  ...
  setRemoteTravel(previous=>{
   ...
   const next=nextItems.map(item=>{
    ...
    return {...item,trackerGuestIds:existing?.trackerGuestIds??[],trackerNames:existing?.trackerNames??[],canTrack:existing?.canTrack??organiser,isCurrentTracker:existing?.isCurrentTracker??false,live:existing?.live??idleLive};
   });
