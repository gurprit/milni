...
export default async function WeddingSectionPage({params}:{params:Promise<{section:string}>}){const {section}=await params;if(!sections.includes(section as Section))notFound();if(section==='travel')return <TravelPage/>;if(section==='menu')return <MenuPage/>;if(section==='events')return <EventsPage/>;if(section==='music')return <MusicPage/>;if(section==='photos')return <PhotosPage/>;if(section==='guests')return <GuestsPage/>;if(section==='singles')return <SinglesPage/>;if(section==='live')return <LivePage/>;return <WeddingDashboard section={section as Section}/>}
