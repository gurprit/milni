...
type PhotoLikeState=Record<string,{count:number;liked:boolean}>;
type ConfettiPiece={id:number;photoId:string;x:number;peak:number;fall:number;r:number;s:number;delay:number;sway:number;drift:number};
...
export default function PhotosPage(){
 ...
 const[photoLikes,setPhotoLikes]=useState<PhotoLikeState>({});
 ...
 useEffect(()=>{
  ...
  void (async()=>{
   try{
    ...
    const shared=(result.photos??[]) as Array<WeddingPhoto&{likeCount?:number;likedByMe?:boolean}>;
    setPhotoLikes(Object.fromEntries(shared.map(photo=>[photo.id,{count:Number(photo.likeCount||0),liked:Boolean(photo.likedByMe)}])));
 ...
 const likePhoto=async(photoId:string)=>{
  const current=photoLikes[photoId]||{count:0,liked:false};
  const nextLiked=!current.liked;
  setPhotoLikes(v=>({...v,[photoId]:{count:Math.max(0,current.count+(nextLiked?1:-1)),liked:nextLiked}}));
  if(nextLiked&&!window.matchMedia('(prefers-reduced-motion: reduce)').matches){
   const pieces=Array.from({length:14},()=>({id:++confettiId.current,photoId,x:(Math.random()-.5)*115,peak:-45-Math.random()*70,fall:115+Math.random()*80,r:(Math.random()>.5?1:-1)*(300+Math.random()*500),s:.6+Math.random()*.75,delay:Math.random()*100,sway:7+Math.random()*10,drift:(Math.random()-.5)*24}));
   setConfetti(v=>[...v,...pieces]);window.setTimeout(()=>setConfetti(v=>v.filter(p=>!pieces.some(n=>n.id===p.id))),6200);
  }
  try{const response=await fetch(`/api/weddings/${encodeURIComponent(slug)}/photos`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'like',photoId})});const result=await response.json();if(!response.ok)throw new Error(result.error||'Could not like photo');setPhotoLikes(v=>({...v,[photoId]:{count:Number(result.likeCount||0),liked:Boolean(result.liked)}}))}catch{setPhotoLikes(v=>({...v,[photoId]:current}))}
 };
 const likeBurst=(photoId:string)=>confetti.filter(p=>p.photoId===photoId).map(p=><i key={p.id} className={styles.likeConfetti} style={{'--cx':p.x+'px','--peak':p.peak+'px','--fall':p.fall+'px','--cr':p.r+'deg','--cs':String(p.s),'--delay':p.delay+'ms','--sway':p.sway+'px','--drift':p.drift+'px'} as React.CSSProperties}><b/></i>);
