...
type LiveUpdate={id:string;title:string;message:string;kind:'update'|'urgent';gif_url:string|null;created_at:string};
...
export default function LivePage(){
 ...
 const [title,setTitle]=useState(''); const [message,setMessage]=useState(''); const [gifUrl,setGifUrl]=useState(''); const [urgent,setUrgent]=useState(false);
 ...
 const post=async()=>{if(!title.trim()||(!message.trim()&&!gifUrl))return;setSaving(true);setError('');try{const r=await fetch(`/api/live-updates/${encodeURIComponent(slug)}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({title,message,gifUrl,kind:urgent?'urgent':'update'})});const x=await r.json();if(!r.ok)throw new Error(x.error||'Could not post update');setUpdates(v=>[x.update,...v]);setTitle('');setMessage('');setGifUrl('');setUrgent(false)}catch(e){setError(e instanceof Error?e.message:'Could not post update')}finally{setSaving(false)}};
 ...
 return <main className={dashboard.shell}><WeddingSidebar base={base} active="live" showOrganiser={modeReady} organiser={organiser} onToggleOrganiser={()=>setMode(!organiser)}/><section className={dashboard.content}>{modeReady&&organiser&&<div className={dashboard.organiserBar}><strong>Organiser mode</strong><span>You’re editing the wedding.</span><button onClick={()=>setMode(false)}>Preview as guest</button></div>}<header className={dashboard.pageHero}><div><small>{names.toUpperCase()}</small><h1>Live updates</h1><p>{formatDateRange(draft.startDate,draft.endDate)} · {draft.city||'Location to be confirmed'}</p></div><Link href={base}>← Wedding home</Link></header><div className={dashboard.pageBody}><div className={styles.layout}>
  <section className={styles.feed}>
   ...
   {loading?<div className={styles.empty}>Loading the latest…</div>:updates.length===0?<div className={styles.empty}><b>All quiet for now.</b><span>When the organisers post an update, it will appear here automatically.</span></div>:<div className={styles.timeline}>{updates.map((u,index)=><article key={u.id} className={u.kind==='urgent'?styles.urgent:''}><div className={styles.when}><strong>{timeLabel(u.created_at)}</strong><span>{dayLabel(u.created_at)}</span></div><div className={styles.marker}><i/></div><div className={styles.updateBody}>{u.kind==='urgent'&&<small>IMPORTANT UPDATE</small>}<h3>{u.title}</h3>{u.message&&<p>{u.message}</p>}{u.gif_url&&<img className={styles.updateGif} src={u.gif_url} alt="Announcement GIF"/>}{organiser&&<button onClick={()=>remove(u.id)}>Remove</button>}</div>{index===0&&<span className={styles.latest}>LATEST</span>}</article>)}</div>}
  </section>
  <aside className={styles.side}>
   {organiser?<section className={styles.composer}><div className={styles.eyebrow}>ORGANISER</div><h2>Post an update</h2><p>Guests with this wedding open will see new posts within a few seconds.</p><label>Headline<input maxLength={80} value={title} onChange={e=>setTitle(e.target.value)} placeholder="Coach leaving in 10 minutes"/></label><label>Message<textarea maxLength={1000} rows={5} value={message} onChange={e=>setMessage(e.target.value)} placeholder="Add the useful details…"/></label><GifPicker selectedUrl={gifUrl} onSelect={setGifUrl} onClear={()=>setGifUrl('')}/><label className={styles.urgentToggle}><input type="checkbox" checked={urgent} onChange={e=>setUrgent(e.target.checked)}/><span><b>Mark as important</b><small>Highlights the update for guests.</small></span></label><button className={styles.postButton} disabled={saving||!title.trim()||(!message.trim()&&!gifUrl)} onClick={post}>{saving?'Posting…':'Post live update →'}</button>{error&&<div className={styles.error}>{error}</div>}</section>:<section className={styles.info}><div className={styles.eyebrow}>STAY IN THE LOOP</div><h2>No frantic group chat required.</h2><p>Keep this page handy during the celebrations. New messages from the organisers appear here automatically.</p><div><b>✦</b><span>Coach movements<br/><small>Departure times and changes</small></span></div><div><b>◇</b><span>Timing updates<br/><small>Know where to be and when</small></span></div><div><b>♡</b><span>Wedding moments<br/><small>Photos, dancing and surprises</small></span></div></section>}
  </aside>
